...
allCaps = runners.map(function(el){
...
largeShirts = runners.filter(function(el){
    if(el.shirt_size == "L"){
        return el
    }
})
...
ticketPriceTotal = runners.map(function(el){
    return el.donation
}).reduce((accum, curnVal)=>{return accum + curnVal}, 0)
...
// Problem 1
//Get the name of the companies the players reprecent

let companies = []

companies.push(runners.map(function(el){
